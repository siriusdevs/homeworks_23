...
def salary_info(
    *departments: tuple[str, dict[str, float]],
    ...
    top3_salary = sorted(salaries)[:3]
    salary_percent = sum(top3_salary) / (sum(salaries) or 1) * 100
    return [round(salary, 2) for salary in top3_salary], round(salary_percent, 2)
